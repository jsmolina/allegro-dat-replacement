...
static const char* detect_type_from_extension(const char* filename) {
    ...
    if (str_iequal(dot, "bmp"))
        return "BMP";
    if (str_iequal(dot, "rle"))
        return "RLE";
    if (str_iequal(dot, "mid") || str_iequal(dot, "midi"))
        return "MIDI";
    if (str_iequal(dot, "wav"))
        return "SAMP";
    if (str_iequal(dot, "fli") || str_iequal(dot, "flc"))
        return "FLIC";
    if (str_iequal(dot, "act") || str_iequal(dot, "pal"))
        return "PAL";
...
static DatObject* find_or_alloc_slot(AllegroDat* dat, DatObject* objs,
    ...
    for (i = 0; i < dat->num_objects; i++) {
        ...
        if (o->num_properties >= 2 && o->properties[1].body && strcmp(o->properties[1].body, new_name) == 0) {
...
static int add_file(AllegroDat* dat, DatObject* objs,
    ...
    if (str_iequal(type_str, "BMP")) {
        ...
        o->body.bmp = bmp;
        init_object_props(o, "BMP ", (s32)(2 + 2 + 2 + (bmp->width * bmp->height * ((u32)bmp->bits_per_pixel / 8u))),
            filepath, datebuf);
        return 1;
    }
    ...
    if (str_iequal(type_str, "FLIC")) {
        ...
        if (sz < 6 || !((buf[4] == 0x11 && buf[5] == 0xAF) || (buf[4] == 0x12 && buf[5] == 0xAF))) {
...
/* ------------------------------------------------------------------ */
/* expand_and_add: expand a glob/wildcard pattern and add all matches */
/* Falls back to treating the pattern as a literal path if no match.  */
/* ------------------------------------------------------------------ */

#ifdef _WIN32
static int expand_and_add(AllegroDat* dat, DatObject* objs,
    const char* pattern, const char* type_override,
    const char* datebuf) {
    WIN32_FIND_DATAA fd;
    HANDLE h;
    char dir[MAX_PATH];
    int added = 0;
    const char *sep1, *sep2, *last_sep;

    sep1 = strrchr(pattern, '/');
    sep2 = strrchr(pattern, '\\');
    last_sep = (!sep1 || (sep2 && sep2 > sep1)) ? sep2 : sep1;
    if (last_sep) {
        size_t dlen = (size_t)(last_sep - pattern) + 1;
        if (dlen >= MAX_PATH)
            dlen = MAX_PATH - 1;
    ...
    if (h == INVALID_HANDLE_VALUE) {
        const char* ts = type_override ? type_override
                                       : detect_type_from_extension(pattern);
        return add_file(dat, objs, pattern, ts, datebuf);
    }
    do {
        char path[MAX_PATH];
        const char* ts;
        ...
        ts = type_override ? type_override : detect_type_from_extension(path);
        if (add_file(dat, objs, path, ts, datebuf))
            added++;
...
static int expand_and_add(AllegroDat* dat, DatObject* objs,
    ...
    for (k = 0; k < gl.gl_pathc; k++) {
        const char* ts = type_override ? type_override
                                       : detect_type_from_extension(gl.gl_pathv[k]);
        if (add_file(dat, objs, gl.gl_pathv[k], ts, datebuf))
            added++;
    }
    globfree(&gl);
    return added;
...
static const char* type_description(const char tag[4]) {
    if (memcmp(tag, "BMP ", 4) == 0)
        return "Bitmap";
    if (memcmp(tag, "PAL ", 4) == 0)
        return "Palette";
    if (memcmp(tag, "RLE ", 4) == 0)
        return "RLE Sprite";
    if (memcmp(tag, "CMP ", 4) == 0)
        return "Compiled Sprite";
    if (memcmp(tag, "XCMP", 4) == 0)
        return "Mode-X Sprite";
    if (memcmp(tag, "FONT", 4) == 0)
        return "Font";
    if (memcmp(tag, "SAMP", 4) == 0)
        return "Sample";
    if (memcmp(tag, "MIDI", 4) == 0)
        return "MIDI";
    if (memcmp(tag, "FLIC", 4) == 0)
        return "FLI/FLC Anim";
    if (memcmp(tag, "DATA", 4) == 0)
        return "Data";
    if (memcmp(tag, "FILE", 4) == 0)
        return "Sub-datafile";
    if (memcmp(tag, "PAT ", 4) == 0)
        return "Gravis Patch";
...
static void print_type_detail(const char tag[4], const u8* body, u32 body_sz) {
    ...
    if ((memcmp(tag, "BMP ", 4) == 0 || memcmp(tag, "CMP ", 4) == 0 || memcmp(tag, "XCMP", 4) == 0) && body_sz >= 6) {
    ...
    if (memcmp(tag, "FONT", 4) == 0 && body_sz >= 2) {
        ...
        if (sz == 8)
            ...
            printf("  proportional (3.9+ format)");
        else if (sz == -1)
            printf("  proportional (legacy)");
        else
            printf("  size=%d", (int)sz);
        return;
    }
...
static int dat_load_existing(const char* filename, DatObject* objs, u32 max_objs) {
    ...
    if (!load_file_bytes(filename, &buf, &bufsz))
        return 0; /* new file, OK */
    if (bufsz < 12) {
        free(buf);
        fprintf(stderr, "Warning: '%s' too small, ignoring\n", filename);
        return 0;
    }
    ...
    for (idx = 0; idx < num_objects && (u32)loaded < max_objs; idx++) {
        ...
        /* Reconstruir propiedades */
        {
            ...
            for (p = 0; p < num_props && scan2 + 12 <= bufsz; p++) {
                ...
                u32 plen;
                memcpy(pr->magic, buf + scan2, 4);
                scan2 += 4;
                memcpy(pr->type, buf + scan2, 4);
                scan2 += 4;
...
int main(int argc, char** argv) {
    ...
    for (i = 2; i < argc; i++) {
        ...
        /* -a PATTERN [files...] : add file(s), glob patterns supported */
        if (strcmp(argv[i], "-a") == 0) {
            if (i + 1 >= argc) {
                fprintf(stderr, "Error: -a requires a file or pattern\n");
                free(objs);
                free(dat);
                return 1;
            }
            /* Consume all consecutive non-option args as patterns/files */
            while (i + 1 < argc && argv[i + 1][0] != '-') {
                i++;
                expand_and_add(dat, objs, argv[i], forced_type, datebuf);
                added_any = 1;
            }
            forced_type = NULL; /* reset after this -a batch */
            continue;
        }
